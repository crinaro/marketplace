...
| `conversations/call_prep_<date>.md` | prep notes for a scheduled call, e.g. `conversations/call_prep_2026-01-15.md`. A note written when full research wasn't available carries `**Prep status:** incomplete — <reason>` under its heading — see below | markdown |
...
and kb files and call preps render as content on their own sections too.
...
- **On a phone:** route 2 — the conversations section of the dashboard renders call preps as content.
