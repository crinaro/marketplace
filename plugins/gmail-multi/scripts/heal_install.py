...
## ⭐⭐ r3 — PLUGIN-NEUTRAL, AND SYNCED (design-manifest-heal.md, § B.1–B.2)

This file used to be jobsearch-only. It is now the CANONICAL half of a byte-identical pair:
`scripts/sync_twins.py` copies it verbatim to `plugins/gmail-multi/scripts/heal_install.py`,
and `scripts/check_twins.py` (wired into CI) fails the build the moment the two diverge. Every
jobsearch-specific literal below is derived from the RUNNING copy's own `.claude-plugin/
plugin.json` instead: the `by` audit string, the CLI's print prefix, the diagnostic logger (a
plugin that ships `_diag.py` uses it; one that does not — gmail-multi — gets a narrower
fallback that appends to `<config>/<name>/diag.jsonl`, never a no-op), and the plugins-dir
resolution (structural, off this file's own installed location, so a `CLAUDE_CONFIG_DIR`
relocation is covered for free — the same derivation `ensure_connectors.derive_install_identity`
already uses). None of this widens ADR-014 decision point 4: the root healed is still always
the running copy's own — see `find_install()` below, unchanged.
...
Usage:
    ...
    python3 heal_install.py --hook     # SessionStart use (gmail-multi's own hooks.json): writes
                                       # a `hook-fired` diag row FIRST, before the registry is
                                       # even opened, then heals; exits 0 on every path
                                       # (ADR-014 decision point 1). jobsearch does not call
                                       # this — `migrate.py --hook` calls `heal_default()`
                                       # in-process, unaffected; this flag exists only because
                                       # the twin is byte-identical to this file.

Exit codes: 0 healthy/healed/repaired/not-installed · 1 unexplained/no-manifest (so it can serve
as a check) — but `migrate.py` calls `heal()` in-process and keeps its own always-0 contract, and
`--hook` (above) always exits 0 regardless of verdict.
...
try:
    ...
    # r3, § B.2c — a plugin that does not ship jobsearch's `_diag.py` (gmail-multi's twin) gets
    # a NARROWER fallback logger, never a no-op: `<config>/<name>/diag.jsonl`, `<config>` derived
    # the same structural way `plugins_dir_default()` derives it below, `<name>` this running
    # plugin's own name. § B.9's `hook-fired` probe depends on this actually writing somewhere
    # queryable — a no-op here would make the probe unable to answer its own question.
    def diag(event, when=None, **fields):
        try:
            config_dir = os.path.dirname(plugins_dir_default())
            name = plugin_name_default(engine_root_default())
            path = os.path.join(config_dir, name, "diag.jsonl")
            rec = {"event": str(event)[:64],
                  "at": when or time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())}
...
def _derive_plugins_dir(root):
    """r3, § B.2d — `<config>/plugins` derived STRUCTURALLY from this engine's own installed
    location, the same spine `ensure_connectors.derive_install_identity()` already reads:
    `<config>/plugins/cache/<marketplace>/<plugin>/<version>/...`. Covers a `CLAUDE_CONFIG_DIR`
    relocation for free, because the config dir is read off the path itself rather than assumed
    to be `~/.claude` — the pre-existing finding this file used to carry. Returns None off that
    spine (a dev checkout, a worktree, a materialized throwaway tree) — never a guess."""
    parts = os.path.realpath(root).split(os.sep)
    for i in range(len(parts) - 3):
        if parts[i] == "plugins" and parts[i + 1] == "cache":
            config_dir = os.sep.join(parts[:i])
            marketplace, plugin = parts[i + 2], parts[i + 3]
            if config_dir and marketplace and plugin:
...
def plugins_dir_default():
    ...
    suite must exercise this against a synthetic plugins dir, never the live install. Below that,
    the structural derivation (`_derive_plugins_dir`, r3); below that, the `~/.claude/plugins`
    literal — unchanged for a copy running off no recognizable cache spine at all."""
    override = os.environ.get("CLAUDESEARCH_PLUGINS_DIR")
    if override:
        return override
    return (_derive_plugins_dir(engine_root_default())
...
def plugin_name_default(root):
    """r3, § B.2a/b — this running copy's own name, read from ITS `.claude-plugin/plugin.json`
    at `root` (never a literal): what makes the `by` audit string and the CLI's print prefix
    plugin-neutral without widening ADR-014 decision point 4 — the root named is always the one
    already being healed, never chosen. `"engine"` only when the manifest cannot be read at all
    (a stray tree `heal()` would already refuse as `not-installed` before this is ever printed)."""
    try:
        with open(os.path.join(root, PLUGIN_JSON), encoding="utf-8") as fh:
            return json.load(fh).get("name") or "engine"
    except Exception:
        return "engine"
...
def _try_clone_repair(root, plugins_dir, plugin_id, entry, doc, disk, recorded, drifted,
    ...
    if manifest_touched:
        ...
        heals.append({"at": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
                      "by": "%s heal_install (public #104)" % plugin_name_default(root), "toVersion": ver,
...
def _heal(root, plugins_dir, apply_it):
    ...
    heals = doc.setdefault("heals", [])
    heals.append({"at": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
                  "by": "%s heal_install (adr-014)" % plugin_name_default(root), "toVersion": ver,
...
def _hook_fired_row(root):
    """r3, § B.3 / § B.9 — the FIRST action of `--hook`, before the registry is even opened:
    proof this hook fired at all, independent of what the heal itself goes on to find. The
    `session_id` comes from the SessionStart payload on stdin, same pattern `journal.py
    --fired`'s `_record_fired` already uses — best-effort, never raises, never blocks the heal
    that follows."""
    session_id = None
    try:
        ...
        pass
    try:
        diag("hook-fired", plugin=plugin_name_default(root), version=disk_version(root),
            root=os.path.realpath(root), session_id=session_id)
...
def main():
    ...
    ap.add_argument("--hook", action="store_true",
                    help="SessionStart use: write a hook-fired diag row first, then heal; "
                         "exits 0 on every path (ADR-014 decision point 1, § B.3/§ B.9)")
    args = ap.parse_args()
    root = os.path.abspath(args.root) if args.root else engine_root_default()
    if args.hook:
        _hook_fired_row(root)
    verdict, lines = heal(root, plugins_dir_default(), apply_it=not args.check)
    name = plugin_name_default(root)
    if lines:
        print("%s: install self-heal (%s)" % (name, verdict))
        print("\n".join(lines))
